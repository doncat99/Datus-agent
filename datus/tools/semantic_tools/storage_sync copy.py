...
class SemanticStorageManager:
    ...
    def _ensure_semantic_model_store(self) -> SemanticModelStorage:
        ...
        if self.semantic_model_store is None:
            # [FIX 1] Import from .store, not .rag
            from datus.storage.semantic_model.store import SemanticModelRAG

            rag = SemanticModelRAG(self.agent_config)
            self.semantic_model_store = rag.storage # Access the storage instance
    ...
    def _ensure_metric_store(self) -> MetricStorage:
        ...
        if self.metric_store is None:
            ...
            self.metric_store = rag.storage # Access the storage instance
    ...
    def _ensure_subject_tree_store(self) -> SubjectTreeStore:
        ...
        if self.subject_tree_store is None:
            # [FIX 2] AgentConfig has no 'db_path'. Use 'rag_storage_path' or base path.
            # Assuming SubjectTreeStore expects a file path or directory.
            db_path = self.agent_config.rag_storage_path()
            self.subject_tree_store = SubjectTreeStore(db_path=db_path)
    ...
    def store_semantic_model(
        ...
        # [FIX 3] Use 'store_batch', not 'batch_store'
        ...
        if dim_objects:
            store.store_batch(dim_objects) # [FIX 3]
        ...
        if measure_objects:
            store.store_batch(measure_objects) # [FIX 3]
        ...
        if identifier_objects:
            store.store_batch(identifier_objects) # [FIX 3]

        logger.info(f"Stored semantic model '{semantic_model_name}'")
    ...
    def store_metric(
        ...
        # MetricStorage uses batch_store_metrics, which is correct
        store.batch_store_metrics([metric_obj])
        logger.debug(f"Stored metric '{metric_data['name']}' with subject path {subject_path}")

    async def sync_from_adapter(self, adapter: Any, **kwargs) -> Dict[str, int]:
        # ... (Rest of sync_from_adapter is logic valid) ...
        # (Included in full file content if needed, but omitted for brevity as errors were above)
        pass